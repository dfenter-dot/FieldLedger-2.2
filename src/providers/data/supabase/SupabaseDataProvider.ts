...
export class SupabaseDataProvider implements IDataProvider {
  ...
  async upsertEstimate(estimate: Partial<Estimate>): Promise<Estimate> {
    const companyId = await this.currentCompanyId();
    const incomingCompanyId = (estimate as any).company_id;
    const normalizedCompanyId = (incomingCompanyId === '' || incomingCompanyId == null) ? companyId : incomingCompanyId;
    const payload = { ...estimate, company_id: normalizedCompanyId, updated_at: new Date().toISOString() };
